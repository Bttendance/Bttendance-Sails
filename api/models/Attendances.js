    ...
    async.until(
      ...
      function (callback) {
        setTimeout(function() { 
          bttendance(values.id, function cb(err, attendance) {
            if (!err && attendance) {
              console.log('Attendance Socket Boradcast : ' + attendance.checked_students);
      ...
      function (err) {
        if (err) {
          ...
          console.log('Attendance afterCreate Done');
      }
    );
