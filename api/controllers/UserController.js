...
module.exports = {
  ...
  signup: function (req, res) {
    ...
    Device.findOneByUuid(device_uuid).populate('owner').exec(function (err, device) {
      if (err)
      ...
      .exec(function (err, user) {
        ...
        if (device) {
          User.create({
            ...
            Device.update({ id: device.id }, { owner: new_user.id }).exec(function (err, updated_device) {
          ...
          Device.create({
            ...
            User.create({
              ...
              Device.update({ id: new_device.id }, { owner: new_user.id }).exec(function (err, updated_device) {
                if (err || !updated_device)
                  return res.send(500, Error.alert(req, "Sign Up Error",  "Deivce Save Error. Please try sign up again."));
            ...
            if (!device) {
              ...
              .create({
                ...
                user.save(function (err, updated_user) {
                  ...
                  User.findOneById(updated_user.id).populateAll().exec(function (err, user) {
                    if (err || !user)
  ...
  update_fullName: function (req, res) {
    res.contentType('application/json; charset=utf-8');
    var email = req.param('email');
    var fullName = req.param('fullName');
  ...
  courses: function (req, res) {
    ...
    .exec(function (err, user) {
      ...
      var total_courses = supervisingCourses.concat(attendingCourses);
          ...
          for (var i = 0; i < courses.length; i++) {
            ...
            for (var j = 0; j < posts.length; j++) {
              ...
              if (posts[j].course == courses[i].id && posts[j].type == "notice") {
                if (posts[j].seen_students.indexOf(user.id) >= 0)
                  notice_seen_count++;
            ...
            if (supervisingCourses.indexOf(courses[i].id) >= 0) {
              ...
              if (notice_last_post)
                notice_unseen = courses[i].students.length - notice_last_post.seen_students.length;
              else
            ...
            courses[i].notice_rate = notice_rate;
